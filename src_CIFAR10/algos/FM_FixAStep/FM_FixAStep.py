...
sys.path.insert(0, os.path.join(ROOT_PATH, 'src_CIFAR10'))
from models.ema import ModelEMA
from load_data import CIFAR10 as dataset
...
parser.add_argument("--CIFAR10ContaminationLevel", "-lvl", default=0, type=int, help="which level of Oliver Contamination setting: [0, 25, 50, 75, 100]")
...
parser.add_argument('--temperature_MM', default=0.95, type=float, help='temperature for MM label guessing')
...
parser.add_argument('--gradient_align_start_iterations', default='0', type=str, help='position at which gradient alignment start') #following MixMatch and FixMatch repo
...
def main(args, brief_summary):
    ...
    class TransformFixMatch(object):
        ...
        def __call__(self, x):
            ...
            return self.normalize(weak1), self.normalize(weak2), self.normalize(strong)
...
# }
    ...
    train_loss_dict['gradient_dot_sign'] = []
    ...
    for epoch in range(args.start_epoch, args.train_epoch):
        ...
        #train
        train_total_loss_list, train_labeled_loss_list, train_unlabeled_loss_unscaled_list, train_unlabeled_loss_scaled_list, train_gradient_dot_sign_list = train_one_epoch(args, l_loader, u_loader, model, ema_model, optimizer, scheduler, epoch)
        ...
        train_loss_dict['gradient_dot_sign'].extend(train_gradient_dot_sign_list)
        ...
        if epoch % args.eval_every_Xepoch == 0:
            ...
            args.writer.add_scalar('train/5.gradient_dot_sign_prob', np.mean(train_gradient_dot_sign_list), epoch)
...
if __name__ == '__main__':
    ...
    experiment_name = "Optimizer-{}_LrSchedule-{}_LrCycleLength-{}_UnlabeledlossWarmupSchedule-NO_UnlabeledlossWarmupIteations-{}_LambdaUMax-{}_MMTemperature-{}_MMAlpha-{}_GradientAlignStartIterations-{}_lr-{}_wd-{}_temperature-{}_mu-{}_threshold-{}".format(args.optimizer_type, args.lr_schedule_type, args.lr_cycle_length, args.unlabeledloss_warmup_iterations, args.lambda_u_max, args.temperature_MM, args.alpha, args.gradient_align_start_iterations, args.lr, args.wd,  args.temperature, args.mu, args.threshold)
    ...
    brief_summary['algorithm'] = 'FM_Combined'
    brief_summary['hyperparameters'] = {
        ...
        'MMTemperature': args.temperature_MM,
        'MMAlpha': args.alpha,
        'gradient_align_start_iterations': args.gradient_align_start_iterations,        
        'lr': args.lr,
        'wd': args.wd,

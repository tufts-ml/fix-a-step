...
# }
    ...
    for epoch in range(args.start_epoch, args.train_epoch):
        ...
        #train
        train_labeled_loss_list = train_one_epoch(args, l_loader, model, ema_model, optimizer, scheduler, epoch)
        ...
        if epoch % args.eval_every_Xepoch == 0:
            ...
            args.writer.add_scalar('train/1.labeled_loss', np.mean(train_labeled_loss_list), epoch)
            ...
            brief_summary["number_of_data"] = {
        "labeled":len(l_train_dataset), "unlabeled":0,
        "validation":len(val_dataset), "test":len(test_dataset)
    }
...
if __name__ == '__main__':
    ...
    experiment_name = "Optimizer{}_LrSchedule{}_LrCycleLength{}_Lr{}_Wd{}_LrWarmupImg{}".format(args.optimizer_type, args.lr_schedule_type, args.lr_cycle_length, args.lr, args.wd, args.lr_warmup_img)

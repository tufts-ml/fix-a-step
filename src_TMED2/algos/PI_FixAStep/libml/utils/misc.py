...
#         labeledtrain_loss = -torch.mean(torch.sum(F.log_softmax(combined_outputs[:batch_size,:], dim=1) * mixed_labeled_target, dim=1))
        ...
        labeledtrain_loss = F.cross_entropy(combined_outputs[:batch_size,:], mixed_labeled_target, weights, reduction='mean')
...
#                 print('{} no grad'.format(name))
        ...
        unlabeledtrain_loss, unlabeled_grads = ssl_obj(inputs_u2, combined_outputs.detach(), model, batch_size)
...
#         print('current_global_iteration is {}'.format(current_global_iteration))
        ...
        p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {total_loss:.4f}. Loss_x: {labeled_loss:.4f}. Loss_u: {unlabeled_loss_unscaled:.4f}. gradient_dot_sign_prob: {gradient_dot_sign_prob:.4f}.".format(
                epoch=epoch + 1,

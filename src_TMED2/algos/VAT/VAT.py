...
#     print('weights used is {}'.format(weights))
    ...
    for epoch in range(args.start_epoch, args.train_epoch):
        ...
        if epoch % args.eval_every_Xepoch == 0:
            ...
            logger.info('At RAW Best val , validation/test %.2f %.2f %.2f' % (best_val_raw_Bacc, best_test_raw_Bacc_at_val))

            logger.info('At EMA Best val, validation/test %.2f %.2f %.2f' % (best_val_ema_Bacc, best_test_ema_Bacc_at_val))
...
if __name__ == '__main__':
    ...
    experiment_name = "Optimizer-{}_LrSchedule-{}_LrCycleLength-{}_UnlabeledlossWarmupSchedule-{}_UnlabeledlossWarmupIteations-{}_LambdaUMax-{}_lr-{}_wd-{}_xi-{}_eps-{}_em-{}_class_weights-{}".format(args.optimizer_type, args.lr_schedule_type, args.lr_cycle_length, args.unlabeledloss_warmup_schedule_type, args.unlabeledloss_warmup_iterations, args.lambda_u_max, args.lr, args.wd,  args.xi, args.eps, args.em, args.class_weights)

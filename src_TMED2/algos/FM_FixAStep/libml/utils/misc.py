...
#         print('after transfering to one-hot, l_labels is {}'.format(l_labels))
#         print('passed in weights is {}'.format(weights))
        
        
        #put data to device
        l_inputs, l_labels = l_inputs.to(args.device).float(), l_labels.to(args.device).long()
        
#         #to handle the soft cross entropy caused by MM
#         weights_this_batch = torch.sum(weights * l_labels, axis=1, keepdims=True)
...
#             print('u_output2 is {}'.format(u_output2))
        ...
        inputs = torch.cat((mixed_labeled_input, u_input_weak1, u_input_strong)).to(args.device)
...
#         logits = de_interleave(logits, 2*args.mu+1)
        ...
        labeledtrain_loss = F.cross_entropy(logits_x, mixed_labeled_target, weights, reduction='mean')
        
#         labeledtrain_loss = -torch.sum(F.log_softmax(logits_x, dim=1) * mixed_labeled_target, dim=1)
#         print('weights_this_batch is {}, labeledtrain_loss is {}'.format(weights_this_batch, labeledtrain_loss))
#         labeledtrain_loss = (labeledtrain_loss * weights_this_batch).mean()/torch.sum(weights_this_batch)
#         print('After using class weights, labeledtrain_loss is {}'.format(labeledtrain_loss))
...
#         print('targets_u is {}'.format(targets_u))
...
#         current_lambda_u = args.lambda_u_max * np.clip(current_global_iteration/(args.unlabeledloss_warmup_pos * args.train_iterations), 0, 1)
        ...
        p_bar.set_description("Train Epoch: {epoch}/{epochs:4}. Iter: {batch:4}/{iter:4}. LR: {lr:.4f}. Data: {data:.3f}s. Batch: {bt:.3f}s. Loss: {total_loss:.4f}. Loss_x: {labeled_loss:.4f}. Loss_u: {unlabeled_loss_unscaled:.4f}. Mask: {mask:.2f}. radient_dot_sign_prob: {gradient_dot_sign_prob:.4f}.".format(
                epoch=epoch + 1,
